...
class TranslationService:
    ...
    def translate_to_pashto(self, text: str) -> Optional[str]:
        """
        Translate English text to Pashto using Deep-translator (Google)
        Preserves PCOS term in English, translates everything else
        
        Args:
            text: English text to translate
            
        Returns:
            Translated Pashto text or None if translation fails
        """
        if not TRANSLATION_AVAILABLE:
            logger.warning("Translation not available - deep-translator not installed")
            return None
        ...
        try:
            # Create a Pashto translator
            pashto_translator = GoogleTranslator(source='en', target='ps')
            
            # Preserve PCOS acronym
            modified_text = text.replace('PCOS', '___PCOS___')
            
            # Translate to Pashto
            translated_text = pashto_translator.translate(modified_text)
            
            if translated_text:
                translated_text = translated_text.replace('___PCOS___', 'PCOS')
                logger.info(f"Pashto translation successful: {len(text)} chars -> {len(translated_text)} chars")
                return translated_text
            else:
                logger.warning("Pashto translation returned empty result")
                return None
            
        except Exception as e:
            logger.error(f"Error during Pashto translation: {e}")
            return None
