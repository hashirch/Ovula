...
from app.core.config import Config, ModelType
...
class LLMService:
    ...
    def get_user_context(self, user_id: int, db: Session) -> str:
        ...
        try:
            ...
            for log in recent_logs:
                context += f"Date: {log.log_date.strftime('%Y-%m-%d')}, "
    ...
    def build_chat_messages(self, system_prompt: str, user_id: int, db: Session,
                            current_message: str, max_history: int = None) -> list:
        """Build a proper message array for Ollama /api/chat with conversation history.
        
        Returns list of {role, content} dicts: [system, user, assistant, ..., user]
        """
...
• آسان، روزمرہ کی اردو استعمال کریں اور چھوٹے پیراگراف بنائیں۔
• NEVER use markdown formatting like asterisks (* or **) or hashtags (#). Use simple dashes (-) for lists. (کسی بھی قسم کی فارمیٹنگ جیسے * یا # استعمال نہ کریں)
...
"""
    ...
    def warmup_model(self):
        """Pre-warm the Ollama model to eliminate first-request cold start"""
        try:
            model = self.config.OLLAMA_BASE_MODEL
    ...
    async def generate_response_ollama(self, prompt: str, model_name: str) -> str:
        """Generate response using Ollama /api/chat. Accepts raw prompt or message list."""
        try:
            ...
            return f"Error: AI service unavailable. Please ensure Ollama is running."
        except Exception as e:
            logger.error(f"Unexpected error in Ollama generation: {e}")
            return f"Error: {str(e)}"

    def stream_ollama_tokens(self, prompt, model_name: str):
        """Generator: yields tokens from Ollama /api/chat. Accepts raw prompt or message list."""
        if isinstance(prompt, list):
            messages = prompt
        else:
            messages = [{"role": "user", "content": prompt}]
...
"""
    ...
    async def generate_response(self, user_message: str, user_id: int, db: Session, model_override: Optional[str] = None, translate_to_urdu: bool = False, translate_to_pashto: bool = False) -> str:
        ...
        try:
            ...
            # Build proper conversation message array with history
            ...
            # Model dispatch — all Ollama models use the chat message array
            model_map = {
                ModelType.OLLAMA_FINETUNED.value: self.config.OLLAMA_FINETUNED_MODEL,
                ModelType.OLLAMA_GEMMA.value: self.config.OLLAMA_GEMMA_MODEL,
                ModelType.OLLAMA_MISTRAL.value: self.config.OLLAMA_MISTRAL_MODEL,
                ModelType.OLLAMA_CODELLAMA.value: self.config.OLLAMA_CODELLAMA_MODEL,
                ModelType.OLLAMA_LLAMA2.value: self.config.OLLAMA_LLAMA2_MODEL,
                ModelType.OLLAMA_DEEPSEEK.value: self.config.OLLAMA_DEEPSEEK_MODEL,
                ModelType.OLLAMA_DEEPSEEK_SMALL.value: self.config.OLLAMA_DEEPSEEK_SMALL_MODEL,
                ModelType.OLLAMA_BASE.value: self.config.OLLAMA_BASE_MODEL,
            }
            
            if current_model_type in model_map:
                response = await self.generate_response_ollama(chat_messages, model_map[current_model_type])
                ...
                # Default fallback to base model
                response = await self.generate_response_ollama(chat_messages, self.config.OLLAMA_BASE_MODEL)
            ...
            if response.status_code == 200:
                ...
                models.extend([
                    ...
                    },
                    {
                        ...
                        "type": ModelType.OLLAMA_DEEPSEEK.value,
                        "available": any(name.startswith("deepseek-r1:7b") for name in model_names)
                    },
                    {
                        "id": "deepseek-small",
                        "name": "DeepSeek R1 (1.5B)",
                        "description": "Fast reasoning model by DeepSeek",
                        "type": ModelType.OLLAMA_DEEPSEEK_SMALL.value,
                        "available": any(name.startswith("deepseek-r1:1.5b") for name in model_names)
                    }
                ])
        except:
